...
\end{tikzpicture}
    \mycaption{Illustration of a neural network \text{[}5, 3\text{]}}{In this network, there are three input features, five neurons in the first hidden layer and 3 neurons in the second hidden layer. Each neuron is activated with a sigmoid function.}
    \label{fig:net}
\end{figure}
...
\begin{figure}[H]
    ...
    \mycaption{Number of firms defaulted, exited for other reasons, surviving each year}{}
    \label{fig:defsurv}
\end{figure}
...
\hline
    \end{tabular}
    \mycaption{Mean of variables for surviving firms, defaulted firms and other exits}{$\Delta$ are one period lagged differences.}
    \label{tab:meanvar}
\end{table}
...
\begin{figure}[]
    ...
    \mycaption{Correlation matrix for firm-specific and macroeconomic covariates}{}
    \label{fig:corrmat}
\end{figure}
...
\begin{figure}
    ...
    \mycaption{Gini coefficient : Linear vs Neural Networks}{}
    \label{fig:ginicomp}
\end{figure}
...
\centering
        ...
        \end{subfigure}
        \mycaption{Sensitivities for each horizon}{}
        \label{fig:sensi}
    \end{figure*}
